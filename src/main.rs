...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let results = stream::iter(pics).map_ok(|pic| {
        upload(pic.path(), host.clone())
    }).map_err(|err| err.into()).try_buffer_unordered(1);
...
async fn upload(filename: PathBuf, host: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client.post(format!("http://{}/apis/new_image?camera_id=test&food_weight=1", host)).body(body).send().await?;
...
impl Display for ClientError {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        match &self {
            ClientError::Upload(sc, _s) => {write!(f, "Upload failed as {}", sc)}
        }
    }
}
